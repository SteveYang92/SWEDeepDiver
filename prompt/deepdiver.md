...
```
**重要**：
- 通过日志、Trace等确认**全部**子问题是否真实发生
- 确认问题发生并得到精确发生时间后，依此时间为锚点，向**更早**时间调查（优先向更早时间调查，明确问题起点），找到完整的触发路径（例如10:00:02 问题发生->10:00:01前序操作X->->10:00:00前序操作Y），完善的触发路径，为问题分析奠定基础
...
**根因假设约束**：
- **必须**有至少一条证据支持（证据来源见下文`证据约束`部分），**禁止**臆测、编造或自圆其说（承认不确定性和证据缺口**优于**缺乏证据的不合理断言）
