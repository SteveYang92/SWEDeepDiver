...
#### 分析流程
```
1.加载知识和预处理文件（问题分析的事实依据和材料）
2.问题确认（精确时间点是什么，是否和描述一致）
3.构造触发路径（起点是什么，前置条件有哪些，中间环节有哪些）
4.提出根因假设（基于已获得的信息，可能的原因有哪些，证据缺口是什么）
5.反复求证假设（证明和排除假设的过程）
6.Review（避免走偏、及时总结和反思）
7.Review不过 → 重复3-6，逐步锁定根因，Review通过 → 锁定根因，进入结论输出阶段
```
...
**1、问题确认约束**：
- 通过日志、Trace等事实确认问题是否真实发生：精确时间点是什么、关键事件和用户描述的对不对得上

**2、问题触发路径约束**：
- 确认问题发生并得到精确发生时间后，依此时间为锚点，向**更早**时间追溯，找到前序事件
- 按时间先后排序事件节点
- 每个节点附原始日志
- 路径有始有终：不能只有终点无起点
- 需要有问题时间点之前的前序事件
...
前序事件包括但不限于：
- 问题前置直接或间接触发条件（用户交互、页面切换、数据加载、接口调用等）
- 环境信息（网络状态、系统负载等）等

**3、根因假设约束**：
- **必须**有至少一条证据支持（证据来源见下文`证据约束`部分）
- **禁止**臆测、编造或自圆其说
- 可正向证实或反向证伪（例如发现了A线索，可直接证明或增强假设1，发现了B线索，可直接排除假设2）
- 满足自洽性：
  - **时间自洽**：根因时间早于现象时间
  - **时间邻近**：根因与现象时间接近（秒级）
  - **因果自洽**：有明确传导路径
